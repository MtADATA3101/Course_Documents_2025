...
**Question: How much time did the flights make up in the air? (Create a new column called gain.)**

```{r mutate gain}

```

You can add more than one column using mutate. You can control where the columns are created using .before and .after (or relocate, which we'll cover today). You can also use the argument .keep = "used" to only keep the columns used in the calculation or created by mutate.

**Question/Task: Create a new dataframe called flights_delay_gain. Add columns for gain, hours, and gain per hour. Only keep the columns used or created in this task.**
...
**Task: Relocate time_hour from the end of the dataframe to the beginning.**
...
Let's consider what this would like without the pipe. Convert to nesting:
...
**Question: What was the longest arrival delay for each month?**
...
Group_by() and mutate() are often used together to add a summary statistic to a dataset. You will usually want to ungroup your data after a calculation to reduce the likelihood of miscalculation later.

**Question: Group by month, add an avg_monthly_delay calculation, and test another calculation with and without ungrouping**
